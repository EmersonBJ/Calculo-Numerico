...
import numpy as np
def f(x):
    return np.float64(math.exp(x) - 2), np.float32(math.exp(x) - 2), math.trunc((math.exp(x) - 2)*10e3)/10e3
def g(x):
    return math.cos(x) - x
...
while abs(f(bb) - f(ab)) > t:  # Erro < tolerancia
    ...
    errors_B_trunc.append(abs(math.trunc(f(c)*10e3)/10e3))
...
#import numpy as np
print(f"Bisseção - f(c) Float 64: {float(f(c)):.20f} | Float 32: {np.float32(f(c)):.20f} | Truncamento - F(C) 4 casas decimais:{math.trunc(f(c)*10e3)/10e3:.20f}") # Float64, float32 e truncamento 4f
print(f"Falsa Posição - h(p) Float 64: {float(h(p)):.20f} | Float 32: {np.float32(h(p)):.20f} | Truncamento - H(p) 4 casas decimais:{math.trunc(h(p)*10e3)/10e3:.20f}")
...
#errors_B_trunc = []   -> errors_B_trunc.append(abs(math.trunc(f(c)*10e3)/10e3))
